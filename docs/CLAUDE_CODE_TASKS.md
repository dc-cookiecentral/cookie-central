...
> as-built state and the current next steps, read **`sample-site/docs/SAMPLE_CENTRAL_STATUS.md`**.
...
`sample-site/docs/SHIPSTATION_SETUP_CHECKLIST.md`: connect Custom Store (endpoint URL + Basic Auth → Vault), status mapping, **launch-blocking** shipping-service mapping (serviceCode 1:1) + automation rules (incl. cold-chain → next-day), cold-chain product tags (co-man), box packages, packing-slip token, email BCC, mapping test via manual import (no label needed), plus test-mode guidance for the internal stress test (§8b) and the go-live cleanup (§9).

### Task 3.5 — Verify, then flip to production. ADR + PR. ✅ (as-built ADR-029)
Verified end-to-end against the live account: a test order imported into ShipStation's **Awaiting Shipment** queue with fields mapped, and a `shipnotify` POST wrote tracking back + advanced `status → shipped`. As-built recorded in **ADR-029** (incl. the XSD corrections: `<Country>US</Country>` + `<OrderTotal>` are required, status exported verbatim, PostgREST `table!fk` embeds). **Current state and next steps: `sample-site/docs/SAMPLE_CENTRAL_STATUS.md`** — that file is authoritative; this one is history. In brief, as of Jul 28 2026: schema/function/frontend all deployed and verified, rush notification rule built, test mode live. Remaining blockers are §3 automation rules and §4 cold-chain tags (the latter now genuinely blocking, since the catalog opened to Raw products). `delivered` and `processing` are **unreachable** — the Custom Store has no such events (ADR-033).
