...
- **Extension track — Spec Sheet → Sample Central → ShipStation** (separate from the Operations/Financials phases below): built July 2026. Phase 1 Cookulator product spine (ADR-025) ✅, Phase 2 Sample Central (ADR-026) ✅, Phase 3 ShipStation Custom Store (ADR-028/029) ✅ **verified end-to-end July 27, 2026**; refined July 28 (ADR-030→033: prototype-aligned shell, rush flag replaces the shipping-speed selector, third-party billing, read-only status, shipnotify cost/label capture). Schema, Edge Function and frontend all deployed.
  ...
  **August 21–23 2026 — three more things shipped.** The **EOS Tracker** went live at `/eos` (ADR-047; `docs/EOS.md`), and its scorecard was reshaped in a pre-launch review: 13 measurables with goals, ranges rather than ceilings where days-on-hand needed a floor, and To-Dos that hang off a measurable and carry forward until ticked. The **Walmart Demand Planner** went live at `/demand-planner`, running on a static 2026-08-13 snapshot — its feeds have no table in the schema yet, and `docs/DEMAND_PLANNER_FORMULAS.md` carries the handoff for wiring it up. And **five pages were hidden pending rework** (Weekly Report, Product Orders, Payments, EOM Snapshot, Lot Trace) — routes intact, nav entries flagged `hidden: true`, internal landing moved to `/inventory`.
